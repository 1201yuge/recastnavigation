...
void CrowdToolState::init(class Sample* sample)
{
	...
	if (nav && crowd && (m_nav != nav || m_crowd != crowd))
	{
		...
		crowd->init(MAX_AGENTS, m_sample->getAgentRadius(), nav);  // 初始化crowd
		
		// Make polygons with 'disabled' flag invalid.
		crowd->getEditableFilter(0)->setExcludeFlags(SAMPLE_POLYFLAGS_DISABLED); // 先失能Polygons
		
		// 初始化障碍物防撞的等级参数(一共四个级别)
...
void CrowdToolState::setMoveTarget(const float* p, bool adjust)
{
	...
	if (adjust)
	{
		// 以最大的速度(ag->params.maxSpeed)朝目标点P方向前行，到达也不会停止！
		float vel[3];
		// Request velocity
		if (m_agentDebug.idx != -1)
		{
	...
	else
	{
		...
		if (m_agentDebug.idx != -1)  // 设置指定的那个Agent目标
		{
			const dtCrowdAgent* ag = crowd->getAgent(m_agentDebug.idx);
			if (ag && ag->active)
...
void CrowdToolState::updateTick(const float dt)
{
	...
	crowd->update(dt, &m_agentDebug);   // 关键地方！！！
